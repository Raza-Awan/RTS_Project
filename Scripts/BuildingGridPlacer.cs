...
public class BuildingGridPlacer : BuildingPlacer
{
    ...
    public void PlaceSelectedBuilding()
    {
        ...
        { // if in build mode
            ...
            if (Physics.Raycast(_ray, out _hit, 1000f, groundLayerMask))
            {
                ...
                if (_toBuild.transform.position.y < 0f)
                {
                    _toBuild.transform.position = new Vector3(_toBuild.transform.position.x, 0f, _toBuild.transform.position.z);
                    //Debug.Log("Y pos = " + _toBuild.transform.position.y, _toBuild.gameObject);
                }
    ...
    public override void _PrepareBuilding()
    {
    ...
    public void _EnableGridVisual(bool on)
    {
